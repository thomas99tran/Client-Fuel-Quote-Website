...
router.get('/', request_handler.check_token, async function(req,res){
...
router.post("/getQuote", async (req, res) => {
    const {value, error} = schemas.getQuoteSchema.validate(req.body)
    ...
    let check = await user.user.findOne({
        ...
        include: [account.account, fuel.fuel]
    })

    if (check.Account.states === "TX") {
        margin_rate += 0.02;
    } else {
        margin_rate += 0.04;
    }

    if (check.Fuels.length) {
        margin_rate -= 0.01;
    }
    ...
    const suggested_price = (price * margin_rate) + price

    res.status(200).json({
        suggested_price: suggested_price.toPrecision(4),
        total_price: (suggested_price * gallons).toPrecision(5),
    })
